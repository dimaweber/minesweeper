...
TEST (Envelope, RoundTripNoLayers) {
  const envelope::envelope_t env(envelope::config_t {.base64 = false});
...
TEST (Envelope, RoundTripBase64Only) {
  const envelope::envelope_t env(envelope::config_t {.base64 = true});
  const auto                 data = to_bytes("hello, world!");
...
TEST (Envelope, RoundTripCompressOnly) {
  const envelope::envelope_t env(envelope::config_t {.base64 = false, .compress = true});
...
TEST (Envelope, RoundTripEncryptOnly) {
  envelope::config_t cfg {.base64 = false, .encrypt = true};
...
TEST (Envelope, RoundTripSignOnly) {
  envelope::config_t cfg {.base64 = false, .sign = true};
...
TEST (Envelope, RoundTripAllLayers) {
  envelope::config_t cfg {.base64 = true, .compress = true, .encrypt = true, .sign = true};
  cfg.encrypt_key = test_encrypt_key( );
  cfg.sign_key    = test_sign_key( );
  const envelope::envelope_t env(cfg);
...
TEST (Envelope, RejectsWrongSignKey) {
  envelope::config_t write_cfg {.base64 = false, .sign = true};
  write_cfg.sign_key = test_sign_key( );
  const envelope::envelope_t writer(write_cfg);

  envelope::config_t read_cfg {.base64 = false, .sign = true};
...
TEST (Envelope, RejectsWrongEncryptKey) {
  envelope::config_t write_cfg {.base64 = false, .encrypt = true};
  write_cfg.encrypt_key = test_encrypt_key( );
  const envelope::envelope_t writer(write_cfg);

  envelope::config_t read_cfg {.base64 = false, .encrypt = true};
...
TEST (Envelope, RejectsMissingSignKeyOnUnwrapOfSignedBlob) {
  ...
  const envelope::envelope_t reader(envelope::config_t {.base64 = false});  // no sign_key configured
...
TEST (Envelope, WrapRejectsEncryptWithoutAValidKey) {
  const envelope::envelope_t env(envelope::config_t {.base64 = false, .encrypt = true});  // encrypt_key left empty
  const auto                 wrapped = env.wrap(to_bytes("payload"));
  EXPECT_FALSE(wrapped.has_value( ));
}

TEST (Envelope, WrapRejectsSignWithoutAKey) {
  const envelope::envelope_t env(envelope::config_t {.base64 = false, .sign = true});  // sign_key left empty
  const auto                 wrapped = env.wrap(to_bytes("payload"));
  EXPECT_FALSE(wrapped.has_value( ));
}
