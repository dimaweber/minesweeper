...
board_t::board_t (std::size_t width, std::size_t height) : data_(width * height), w_ {width}, h_ {height} {
...
size_t board_t::coord_to_index (int x, int y) const {
  if ( valid_coords(x, y) )
    return (y - 1) * w_ + (x - 1);
  throw std::out_of_range(fmt::format("Coordinates ({},{}) are out of range for field size {}x{}", x, y, w_, h_));
}

size_t board_t::coord_to_index (coord_t coords) const {
  return coord_to_index(coords.first, coords.second);
}

auto clients_t::add (client_id_t id, board_id_t field_id, board_t field) {
  const std::lock_guard lock {access_};
  return data_.emplace(id, client_context_t {.board_id_ = field_id, .board_ = field});
}
...
bool board_t::valid_x (int x) const noexcept {
  return x > 0 && x <= static_cast<int>(w_);
}

bool board_t::valid_y (int y) const noexcept {
  return y > 0 && y <= static_cast<int>(h_);
}

bool board_t::valid_coords (int x, int y) const noexcept {
  return valid_x(x) && valid_y(y);
}

bool board_t::valid_coords (coord_t coords) const noexcept {
  return valid_coords(coords.first, coords.second);
}

u_short board_t::operator[] (int x, int y) const {
  return data_[coord_to_index(x, y)];
}

u_short& board_t::operator[] (int x, int y) {
  return data_[coord_to_index(x, y)];
}

u_short board_t::operator[] (coord_t coords) const {
  return data_[coord_to_index(coords)];
}

u_short& board_t::operator[] (coord_t coords) {
  return data_[coord_to_index(coords)];
}
...
auto board_t::begin ( ) const {
  return data_.cbegin( );
}

auto board_t::end ( ) const {
  return data_.cend( );
}

bool board_t::is_boom (ushort cell) {
  return (cell & 0x0a) == 0x0a;
}

bool board_t::is_boom (int x, int y) const {
  if ( !valid_coords(x, y) )
    return false;
  return board_t::is_boom(data_[coord_to_index(x, y)]);
}

bool board_t::is_boom (coord_t coords) const {
  return is_boom(coords.first, coords.second);
}

bool board_t::is_flag (ushort cell) {
  return (cell & 0x20) == 0x20;
}

bool board_t::is_flag (int x, int y) const {
  if ( !valid_coords(x, y) )
    return false;
  return board_t::is_flag(data_[coord_to_index(x, y)]);
}

bool board_t::is_flag (coord_t coords) const {
  return is_flag(coords.first, coords.second);
}

bool board_t::is_revealed (ushort cell) {
  return (cell & 0x10) == 0x10;
}

bool board_t::is_revealed (int x, int y) const {
  if ( !valid_coords(x, y) )
    return false;
  return board_t::is_revealed(data_[coord_to_index(x, y)]);
}

bool board_t::is_revealed (coord_t coords) const {
  return is_revealed(coords.first, coords.second);
}

void board_t::toggle_flag (ushort& cell) {
  cell ^= 0x20;
}

void board_t::toggle_flag (int x, int y) {
  if ( !valid_coords(x, y) )
    return;
  board_t::toggle_flag(data_[coord_to_index(x, y)]);
}

void board_t::toggle_flag (coord_t coords) {
  toggle_flag(coords.first, coords.second);
}

void board_t::mark_revealed (ushort& cell) {
  cell |= 0x10;
}

void board_t::mark_revealed (int x, int y) {
  if ( !valid_coords(x, y) )
    return;
  board_t::mark_revealed(data_[coord_to_index(x, y)]);
}

void board_t::mark_revealed (coord_t coords) {
  mark_revealed(coords.first, coords.second);
}

std::vector<coord_t> board_t::neighbors (int x, int y) const {
...
std::vector<coord_t> board_t::neighbors (coord_t coords) const {
  return neighbors(coords.first, coords.second);
}

int board_t::reveal (int x, int y) {
  mark_revealed(x, y);
  return neighbor_bombs_count(x, y);
}

int board_t::reveal (coord_t coords) {
  return reveal(coords.first, coords.second);
}

int board_t::neighbor_bombs_count (int x, int y) const {
  if ( !valid_coords(x, y) ) {
    return -1;
  }
  if ( is_boom(x, y) ) {
...
int board_t::neighbor_bombs_count (coord_t coords) const {
  return neighbor_bombs_count(coords.first, coords.second);
}

int board_t::neighbor_flags_count (int x, int y) const {
  if ( !valid_coords(x, y) ) {
    return -1;
  }
  return std::ranges::count_if(neighbors(x, y), [this] (const auto& p) { return this->is_flag(p); });
}
int board_t::neighbor_flags_count (coord_t coords) const {
  return neighbor_flags_count(coords.first, coords.second);
}

int board_t::neighbor_revealed_count (int x, int y) const {
  if ( !valid_coords(x, y) ) {
    return -1;
  }
  return std::ranges::count_if(neighbors(x, y), [this] (const auto& p) { return this->is_revealed(p); });
}

int board_t::neighbor_revealed_count (coord_t coords) const {
  return neighbor_revealed_count(coords.first, coords.second);
}

int board_t::neighbor_unrevealed_count (int x, int y) const {
  if ( !valid_coords(x, y) ) {
    return -1;
  }
  return std::ranges::count_if(neighbors(x, y), [this] (const auto& p) { return !this->is_revealed(p); });
...
int board_t::neighbor_unrevealed_count (coord_t coords) const {
  return neighbor_unrevealed_count(coords.first, coords.second);
}

int board_t::bombs_total ( ) const {
  return std::ranges::count_if(data_, [] (u_short cell) { return board_t::is_boom(cell); });
}

int board_t::flags_count ( ) const {
  return std::ranges::count_if(data_, [] (u_short cell) { return board_t::is_flag(cell); });
}
...
int board_t::unrevealed_count ( ) const {
  return std::ranges::count_if(data_, [] (u_short cell) { return !board_t::is_revealed(cell); });
}
...
std::optional<client_id_t> addon_api_t::add_new_client (board_id_t board_id) {
  ...
  const auto [it, ok]  = clients.add(id, board_id, boards.at(board_id));
  if ( !ok ) {
...
board_t* addon_api_t::board_for_client (client_id_t client_id) {
...
response_t& response_t::add_property (const std::string& key, parameter_t value) {
