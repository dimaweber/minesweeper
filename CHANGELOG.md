...
- Regenerate the live-replay fixture against the new error-message casing (`24dcbcc`). The
  ...
  warnings, not failures, since they don't touch the `"status"` field `live_replay`'s
  outcome check cares about, but a permanent, intentional change deserves an updated
  fixture rather than a standing warning on every run. Regenerated by replaying the
  existing fixture's request sequence against a real server and capturing its own
  `requests.log` as the new fixture, verified via a timestamp/token-normalized diff that
  the only substantive changes are exactly those 3 lines. `live_replay` now reports zero
  warnings.
